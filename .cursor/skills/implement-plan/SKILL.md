...
description: >-
  ...
  Use when the user types `/implement-plan #123` with the GitHub issue number, or asks
...
- **Required:** Issue number or issue URL (must yield a single issue id), as in **`/implement-plan #123`**.
- **Optional:** Path to the accepted plan file, or short constraints from the human (deadline, out of scope).

## Steps

1. Parse **`#n`**. Every delegation to **`coding-clanker`** must include **`#n`** in the Task prompt so hooks and labels can run (see [coding-clanker parent handoff](@.cursor/agents/coding-clanker.md)).
...
- If implementation needs another pass after review, the human can run **`/implement-plan #n`** again on the same branch.
