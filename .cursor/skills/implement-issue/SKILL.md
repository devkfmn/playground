...
2. Prepare git state before implementation:
  - Checkout `**dev`**.
  - Fetch `**origin`**.
  - Pull `**origin/dev`**.
  - Create the feature branch (suggested name: `**feature/issue-<n>-short-slug`**) unless the human explicitly asks to reuse an existing feature branch.
  - Switch to that feature branch.
3. Collect context for `**coding-clanker`**: accepted plan text or path, issue **#n**, and the prepared feature branch name.
4. **Delegate `coding-clanker`** via **Task** with `subagent_type` `**coding-clanker`**. Do **not** replace the subagent with inline implementation unless delegation is impossible.
5. Remind `**coding-clanker`**: follow the plan and [AGENTS.md](@AGENTS.md); run `**npm run build`** from [package.json](@package.json); do **not** commit, push, or open or update a PR.
6. Update issue labels explicitly after successful delegation:
